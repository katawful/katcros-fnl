  ...
  (let [opts# {}
    ...
    (if (= (type desc) :table)
        (do
          ...
          `(vim.api.nvim_create_autocmd ,events ,desc)) ; if a desc string, add them all to the opts table
        (= (type desc) :string)
        (do
          (tset opts# :desc desc)
          ...
          (if (not= args nil)
              (each [key val (pairs args)]
                (tset opts# key val)))
          `(vim.api.nvim_create_autocmd ,events ,opts#)) ; no additional options or description
        (do
          (if (not= pattern nil)
              (tset opts# :pattern pattern))
          (tset opts# call-type# callback)
...
(fn aug- [group ...]
  ...
  (let [autocmd# [...]
        out# []]
    (each [key# val# (ipairs autocmd#)] ; (assert-compile (list? val#) "expected list for body" val#)
      (assert-compile (if (= (tostring (. val# 1)) :auc-)
                          :auc-) "expected autocmd call"
                      (. val# 1))
      (if (= (tostring (. val# 1)) :auc-)
          (do
            ; see if have table
            (var had-table# false)
            (each [k# v# (pairs val#)] ; gotta make sure its an actual table ; not a list or symbol
              (if (and (not= k# 2) (= (type v#) :table) (not= (list? v#) v#)
                       (not= (sym? v#) v#))
                  (do
                    (tset v# :group group)
                    (set had-table# true))))
...
(fn del-aug! [id]
  "Macro -- delete augroup by id or name"
  (assert-compile (or (= (type id) :string) (= (type id) :number))
